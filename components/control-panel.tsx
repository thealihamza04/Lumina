...
export function ControlPanel({
  ...
  return (
    <div className="flex flex-col gap-6 h-full text-[#d8d2f6]">
      {/* Layer Type & Name */}
      <div className="space-y-4">
        <div>
          <label className="text-xs font-medium text-[#9f95c5] mb-1.5 block">Layer Name</label>
          <input
            type="text"
            value={layer.name}
            onChange={(e) => onUpdateLayer({ ...layer, name: e.target.value })}
            className="w-full px-3 py-2 border border-[#4c4561] bg-[#2a2538] rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-[#8b7dff]"
          />
        </div>

        <div>
          <label className="text-xs font-medium text-[#9f95c5] mb-1.5 block">Layer Type</label>
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className={`flex items-center justify-center gap-2 px-3 py-2 rounded text-sm font-medium transition ${layer.type === 'gradient'
                ? 'bg-[#8b7dff] text-[#130f1d] shadow-sm'
                : 'bg-[#2a2538] text-[#bcb3df] hover:bg-[#332c47]'
                }`}
            >
      ...
      {/* Layer Appearance */}
      <div className="space-y-4 border-t border-[#3f3a4f] pt-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-medium text-[#9f95c5] mb-1 block">Opacity: {Math.round(layer.opacity * 100)}%</label>
            <Slider
              min={0}
              max={1}
              step={0.01}
          ...
          <div>
            <label className="text-xs font-medium text-[#9f95c5] mb-1 block">Blend Mode</label>
            <select
              value={layer.blendMode}
              onChange={(e) => onUpdateLayer({ ...layer, blendMode: e.target.value as any })}
              className="w-full px-2 py-1 border border-[#4c4561] bg-[#2a2538] rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-[#8b7dff]"
            >
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <label className="text-xs font-medium text-[#9f95c5]">Blur: {layer.blurAmount}px</label>
            <button
              onClick={() => onUpdateLayer({ ...layer, blurEnabled: !layer.blurEnabled })}
              className={`w-8 h-4 rounded-full transition-colors relative ${layer.blurEnabled ? 'bg-[#8b7dff]' : 'bg-[#4b4560]'
                }`}
            >
              <div
          ...
          {/* Noise Effect */}
          <div className="flex items-center justify-between">
            <label className="text-xs font-medium text-[#9f95c5]">Noise: {layer.noiseAmount}%</label>
            <button
              onClick={() => onUpdateLayer({ ...layer, noiseEnabled: !layer.noiseEnabled })}
              className={`w-8 h-4 rounded-full transition-colors relative ${layer.noiseEnabled ? 'bg-[#8b7dff]' : 'bg-[#4b4560]'
                }`}
            >
              <div
      ...
      {layer.type === 'color' && (
        <div className="border-t border-[#3f3a4f] pt-4">
          <label className="text-sm font-semibold text-[#d8d2f6] mb-2 block">Layer Color</label>
          <div className="flex gap-3 items-center">
            ...
            <input
              ...
              value={layer.color}
              onChange={(e) => onUpdateLayer({ ...layer, color: e.target.value })}
              className="flex-1 px-3 py-2 border border-[#4c4561] bg-[#2a2538] rounded-xl text-sm font-mono"
            />
          </div>
        </div>
      )}
      ...
      {layer.type === 'gradient' && gradient && (
        <div className="space-y-6 border-t border-[#3f3a4f] pt-4">
          {/* Gradient Type */}
          <div>
            <h3 className="text-sm font-semibold text-[#d8d2f6] mb-2">Gradient Type</h3>
          ...
          {(gradient.type === 'linear' || gradient.type === 'linear-repeating') && (
            <div>
              <label className="text-xs font-medium text-[#9f95c5] mb-1.5 block">Angle: {gradient.angle}°</label>
              <Slider
                min={0}
                max={360}
                step={1}
          ...
          {(gradient.type === 'radial' || gradient.type === 'radial-repeating') && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <h4 className="text-[10px] font-bold text-[#8e84b4] uppercase mb-1">Shape</h4>
                ...
                <div>
                  <h4 className="text-[10px] font-bold text-[#8e84b4] uppercase mb-1">Size</h4>
                  <select
                    value={gradient.radialSize}
                    onChange={(e) => updateGradient({ radialSize: e.target.value as any })}
                    className="w-full px-1 py-1 border border-[#4c4561] bg-[#2a2538] rounded text-[10px]"
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-bold text-[#8e84b4] uppercase block">Pos X: {gradient.radialX}%</label>
                  <Slider
                    min={0}
                    max={100}
                    step={1}
                ...
                <div>
                  <label className="text-[10px] font-bold text-[#8e84b4] uppercase block">Pos Y: {gradient.radialY}%</label>
                  <Slider
                    min={0}
                    max={100}
                    step={1}
          ...
          {(gradient.type === 'conic' || gradient.type === 'conic-repeating') && (
            <div className="space-y-4">
              <div>
                <label className="text-[10px] font-bold text-[#8e84b4] uppercase block">Start Angle: {gradient.conicAngle}°</label>
                <Slider
                  min={0}
                  max={360}
                  step={1}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-bold text-[#8e84b4] uppercase block">Center X: {gradient.conicX}%</label>
                  <Slider
                    min={0}
                    max={100}
                    step={1}
                ...
                <div>
                  <label className="text-[10px] font-bold text-[#8e84b4] uppercase block">Center Y: {gradient.conicY}%</label>
                  <Slider
                    min={0}
                    max={100}
                    step={1}
          ...
          <div className="pt-2">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-[#d8d2f6]">Color Stops</h3>
              <Button
                onClick={addStop}
                size="sm"
                variant="outline"
